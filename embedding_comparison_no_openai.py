...
def main():
    ...
    print(f"\n>> Global korrelationsmatris (alla {len(concat_sims[anchor])} par):")
    print(global_corr_matrix.round(4))
    global_corr_matrix.to_csv("global_model_correlations.csv")
    ...
    if summary_rows:
        ...
        avg_performance.to_csv("average_model_performance.csv")
    ...
    print(f"\n=== Analys klar! ===")
    print("Filer skapade:")
    print("- spearman_vs_gold_summary.csv: Detaljerade resultat per dataset")
    print("- average_model_performance.csv: Genomsnittlig prestanda per modell")
    print("- global_model_correlations.csv: Global korrelationsmatris")
    print("- model_correlations_[dataset].csv: Korrelationer per dataset")
